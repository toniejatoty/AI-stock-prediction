        ...
        days_in_past_to_train.update(
            {
                i: int_max_value
                for i in range(
                    min(1000, max_size_df - days_in_future_to_predict), 200, -50
                )
            }
        )
    if max_size_df > 50:
        days_in_past_to_train.update(
            {
                i: int_max_value
                for i in range(
                    min(200, max_size_df - days_in_future_to_predict), 50, -10
                )
            }
        )
...
def get_prediction(
    df_org, days_to_train, days_to_test_model, model, days_in_future_to_predict
):
