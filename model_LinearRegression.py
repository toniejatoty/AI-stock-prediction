...
def predict_stock_prices(df_org, days_in_future_to_predict, loss_function):
    ...
    _,_,result_of_testing=get_prediction(df_org.iloc[0:-days_in_future_to_predict],days_to_train_testing,days_in_future_to_predict)
    ...
    #to predict future
    _,_,future_pred=get_prediction(df_org,days_to_train_pred,days_in_future_to_predict)
    print(days_to_train_testing)
    return ( result_of_testing,future_pred,days_to_train_pred,score, Status )



def get_prediction(df_org, days_to_train, days_in_future_to_predict):
    ...
    X_future = np.array(range(days_to_train+days_in_future_to_predict, days_to_train+(2*days_in_future_to_predict))).reshape(-1, 1)
    modelfuture = LinearRegression()
    modelfuture.fit(X_test, y_test)
    return model.predict(X_test), y_test, modelfuture.predict(X_future)
...
def get_best_num_of_days_to_train(df_org, days_in_future_to_predict,loss_function):
    ...
    days_in_past_to_train.update({i: infinity for i in range(min(10, max_size_df - days_in_future_to_predict), 1, -1)})
    for days_to_train in days_in_past_to_train:
        x_train_pred, y_train, future_pred = get_prediction(df_org,days_to_train,days_in_future_to_predict)
