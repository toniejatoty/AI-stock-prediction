...
def run_model(ticker, days_to_predict, start_date, days_to_train, epochs, 
              loss_function, optimizer_name, learning_rate, batch_size, num_layers, *args):
    global should_stop
    should_stop = False
    ...
    lstm_layers = []
    params_per_layer = 6
    num_layers = int(num_layers)  # Upewniamy się, że to liczba
    
    # Pobieramy tylko parametry dla aktywnych warstw
...
with demo:
    ...
    start_button.click(
        fn=run_model,
        inputs=[ticker, days_to_predict, start_date, days_to_train, epochs,
                loss_function, optimizer_name, learning_rate, batch_size, num_layers] +  # Dodajemy num_layers
